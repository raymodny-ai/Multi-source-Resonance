...
def make_snapshot(
    ...
    for i in range(n_strikes):
        ...
        # IV violations: 让 call_iv 和 put_iv 偏差 > 2%
        if has_iv and (i / max(1, n_strikes)) < iv_violation_ratio:
            call_iv = base_iv
            put_iv = base_iv + 0.05  # 5% 偏差
        else:
            call_iv = base_iv
            put_iv = base_iv
        strikes.append({
...
def test_perfect_snapshot_scores_1():
    ...
    # OI 覆盖率会跑 yfinance, 测试时没网 → 应该是 None, 不影响 score
...
def test_high_zero_oi_deduct():
    ...
    print("=" * 60)
    v = GEXDataQualityValidator()
    snap = make_snapshot(n_strikes=100, zero_oi_ratio=0.5)  # 50% 零 OI
    result = v.validate_snapshot('SPY', snap)
...
def test_iv_violations_deduct():
    ...
    print("=" * 60)
    v = GEXDataQualityValidator()
    snap = make_snapshot(n_strikes=100, iv_violation_ratio=0.20)  # 20% 违例
    result = v.validate_snapshot('SPY', snap)
...
def test_combo_drops_below_threshold():
    ...
    snap = make_snapshot(
        n_strikes=30,  # 稀疏
        timestamp=old_ts,  # 陈旧
        zero_oi_ratio=0.4,  # 零 OI 多
        iv_violation_ratio=0.10,  # IV 不一致
    )
    del snap['data']['spot_price']
    result = v.validate_snapshot('SPY', snap)
    print(f"  score={result['score']}, valid={result['valid']}, issues={result['issues']}")
